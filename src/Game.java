...
public class Game {
        ...
        try {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                if (parts[0].equals("state")) {
                    ...
                    states.add(parts[1], first); // add to the dictionary in State
                if (current_state == null) { // only set the first state current_state
                    current_state = first;
                    }
                } else if (parts[0].equals("item")) {
                    Item item = new Item(parts[1], parts[2], parts[3]);
                    // add to items dictionary in state class
                    State location = states.get(parts[3]); // get the location of the item returning state object
                    location.items.add(parts[1], item); // add item to the dictionary of the corresponding state
                    items.add(parts[1], item); // add item to the game item dictionary
                } else if (parts[0].equals("command")) {
                // refers to either state or item?
                    Command command = new Command(parts[1], parts[2], parts[3], parts[4]);
                    State fromLocation = states.get(parts[2]); // items go in here
                    fromLocation.commands.add(parts[1], command); // add that command to the command dictionary of the corresponding state
    ...
    public void print(State current_state) {
        if (current_state != null) {
            System.out.print("\n" + "You are in the " + current_state.name.toUpperCase() + ". " + "\n");
            ...
            if (current_state.items.keys != 0) {
                for (int i = 0; i < current_state.items.table.length; i++) {
                    ...
                    while (current != null) {
                        System.out.print("There is a " + current.key + ", " + current.value.description + " here. " + "\n");
    ...
    public void play() {
        ...
        System.out.print("You are alone brushing your teeth late at night. You are tired from a long day of coding.  Suddenly, you hear footsteps behind you. You look in the mirror and see a woman wearing a long, white dress. IT'S LILLIAN, THE GHOST OF MERION! NOW RUN FOR YOUR LIFE!"+ "\n");
        System.out.print("Type 'help' for instructions." + "\n");
        print(current_state);

        while (true) {
            String prompt = " >> ";
            String input = this.getInput(prompt); // input added by user AFTER the prompt
            Command command = current_state.commands.get(input); // return the command associated
            ...
            if (input.length() >= 4 && input.substring(0, 4).equals("drop")) {
                if (input.length() == 4) {
                    ...
                    if (item == null) {
                        System.out.print(input.substring(5, input.length()) + " is not an item." + "\n");
                    } else if (backpack.contains(item.name)) { // if input is NOT an item, don't try to put in contains or else nullpointerexception
                        // remove item from backpack
                        backpack.delete(item.name);
                        System.out.print(item.name + " has been dropped from the backpack." + "\n");
                        current_state.items.add(item.name, item); // add it to the current state
                    } else {
                ...
                if (input.length() == 7) {
                    ...
                    if (item != null) {
                        if (current_state.items.contains(item.name)) {
                            ...
                            current_state.items.delete(item.name); // remove the item the state
                ...
                System.out.print("Here are the items in this state that are missing from your backpack: " + needed + "\n");
                ...
                if (input.length() == 5) {
                    ...
                    if (toState != null) {
                        ...
                        if (toState.name.equals("end")) {
                            System.out.print("CONGRATULATIONS, YOU HAVE SURVIVED!" + "\n");
                        }
                        print(current_state);
                    } else {
                        System.out.print("I'm sorry, but that location doesn't exist...at least in this nightnmare." + "\n");
                    }
                }
            ...
            } else if (command == null) { //if it isn't a command
                System.out.print("Sorry, that command doesn't exist." + "\n");
                moves();
            } else { //commands written into text file, like "go north"
                Node<String> current = command.parsed.list; //pointer to compare all items in parsed, go through all items associated with that command
                while (current != null) {
                    // check that bpcontains has all of the needed items
                    if (backpack.contains(current.key)) {
                        bpcontains = true;
                    } else {
                        bpcontains = false;
                        break; // leave as soon as a needed item is not in backpack
                    }
                    current = current.next;
                }
                if (bpcontains == true) { // search the current_state for that command) {
                    State toState = states.get(command.to_state); // return the state associated with that command
                    current_state = toState;
                    if (current_state.name.equals("end")) {
                        System.out.print("CONGRATULATIONS, YOU HAVE SURVIVED!" + "\n");
                        System.out.print(current_state.description + "\n");
                    } else {
                        print(current_state);
    ...
    public void moves() {
        moves--;
        if (moves > 0) {
            System.out.print("HURRY UP! Lillian is only " + moves + " steps behind you." + "\n");
        } else if (moves == 0) {
    ...
    public boolean search(String current_state, String end_state) {
        ...
        while (!queue.isEmpty()) {
            current = queue.pop_front(); // starting at new branch
            if (current.name.equals(end_state)) {
                System.out.print("Shortest path:" + "\n" + current_state + current.path + "\n"); // print the path leading back to start from end
                found = true;
                break;
            }
            if (!visited.contains(current.name)) {
                visited.add(current.name, current);
                for (int i = 0; i < current.commands.table.length; i++) { // search through all the children
                    NodeHT<String, Command> pointer = current.commands.table[i].list;
                    while (pointer != null) { // iterating through current branch
                        State tostate = states.get(pointer.value.to_state);
                        tostate.parent = current; // set parent to current
                        tostate.path = tostate.parent.path + ", " + pointer.value.direction + " with "+ pointer.value.needed + "\n" + " TO " + tostate.name;
                        queue.append_end(tostate);
                        pointer = pointer.next; // go through next item in commands hashTable of current
                    }
                }
            }
        }
    ...
    public static void main(String[] args) {
        Game game = new Game("src/gameTEXT");
        game.play();
    }
